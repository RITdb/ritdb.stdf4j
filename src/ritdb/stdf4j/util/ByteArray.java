...
public class ByteArray {
  ...
  public ByteArray(ByteOrder byteOrder) { setByteOrder(byteOrder); }
  ...
  public final char toChar(final byte[] bytes) {
    if(byteOrder == ByteOrder.BIG_ENDIAN) return (char)((bytes[1] & 0xFF) + ((bytes[0] & 0xFF) << 8));
    else return (char)((bytes[0] & 0xFF) + ((bytes[1] & 0xFF) << 8));
  }
  ...
  public final int toInt(final byte[] bytes) {
    if(bytes.length == 4) {
      if(byteOrder == ByteOrder.BIG_ENDIAN) return (int)(((bytes[0] & 0xFF) << 24) + ((bytes[1] & 0xFF) << 16) + ((bytes[2] & 0xFF) << 8) + (bytes[3] & 0xFF));
      else return (int)((bytes[0] & 0xFF) + ((bytes[1] & 0xFF) << 8) + ((bytes[2] & 0xFF) << 16) + ((bytes[3] & 0xFF) << 24));
    }
    else return 0;
  }
  ...
  public int toSigned(final byte[] bytes, int length) {
    ...
    if(length == 1 && bytes.length >= 1) value=(int)bytes[0];
    else if(length == 2 && bytes.length >= 2) value=(int)toShort(bytes);
    else value=toInt(bytes);
  ...
  public final long toLong(final byte[] bytes) {
    if(bytes.length == 8) {
      if(byteOrder == ByteOrder.BIG_ENDIAN) return (long)(((bytes[0] & 0xFFL) << 56) + ((bytes[1] & 0xFFL) << 48) + ((bytes[2] & 0xFFL) << 40) + ((bytes[3] & 0xFFL) << 32)
          + ((bytes[4] & 0xFFL) << 24) + ((bytes[5] & 0xFFL) << 16) + ((bytes[6] & 0xFFL) << 8) + (bytes[7] & 0xFFL));
      else return (long)((bytes[0] & 0xFFL) + ((bytes[1] & 0xFFL) << 8) + ((bytes[2] & 0xFFL) << 16) + ((bytes[3] & 0xFFL) << 24) + ((bytes[4] & 0xFFL) << 32) + ((bytes[5] & 0xFFL) << 40)
  ...
  public final short toShort(final byte[] bytes) {
    if(bytes.length == 2) {
      if(byteOrder == ByteOrder.BIG_ENDIAN) return (short)((bytes[1] & 0xFF) + ((bytes[0] & 0xFF) << 8));
      else return (short)((bytes[0] & 0xFF) + ((bytes[1] & 0xFF) << 8));
    }
    else return 0;
  }
