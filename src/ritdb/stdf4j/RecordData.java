...
public class RecordData {
  ...
  public boolean notAtEnd(String name){
	  // true if this field is past the end of the available data
	  return (desc.getIndex(name) < fields.length);
  }
  ...
  public String getString(String name) {
    ...
    else if(value instanceof Double) repr=String.format("%f", (Double)value);
    else if(value instanceof Float) repr=String.format("%f", (Float)value);
    else if(value instanceof Byte) repr=String.format("%x", (Byte)value);
    else if(value instanceof byte[]) {
      StringBuilder bb=new StringBuilder("[");
      for(byte b : (byte[])value) {
        if(bb.length() > 1) bb.append(", ");
        bb.append(Byte.toString(b));
      }
      bb.append("]");
      repr=bb.toString();
    }
    else if((name.endsWith("_T") && !name.equals("TEST_T")) || name.equals("MOD_TIM")) {
      Calendar cal=Calendar.getInstance();
      //cal.setTimeZone(TimeZone.getTimeZone("GMT"));
      cal.setTimeInMillis((Long)value * 1000L);
      repr=String.format("%1$ta %1$tb %1$td %1$tT %1$tY", cal);
    }
    else repr=value.toString();

    if(repr.length() == 0) repr="(null)";
  ...
  public String toString() {
    ...
    for(String name : names) {
      sb.append("   ").append(name).append(": ").append(getString(name)).append("\n");
    }
