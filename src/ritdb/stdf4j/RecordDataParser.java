...
public class RecordDataParser {
  ...
  public RecordDataParser(RecordDescriptor desc, int pos, byte[] data, ByteOrder byteOrder) {
    this.desc=desc;
    this.pos=pos;
    this.data=data;
  ...
  public RecordData parse() throws ParseException {
    ...
    for(Field field : desc.getFields()) {
      ...
      if(available > 0 && validField(field, fieldList)) {
        ...
        value=readField(field.getType(), field.getLength(), field.getLengthFieldIndex(), field.getArrayType(), field.getArraySizeFieldIndex(), fieldList);
      }
     /* else {
        char type=field.getType();
        if(type == 'U' || type == 'I') value=Integer.valueOf(0);
        else if(type == 'R') value=Double.valueOf(0.0);
        else if(type == 'k') value=readArray(field.getArrayType(), field.getArraySizeFieldIndex(), field.getLengthFieldIndex(), field.getLength(), fieldList, true);
        else if(type == 'B' && field.getLength() == 1) value=Byte.valueOf((byte)0);
      }
  */
...
//TODO extend to U8 here or some where
  protected Object readField(char type, int length, int lengthFieldIndex, char arrayType, int arraySizeFieldIndex, Object[] fields) throws ParseException {
    switch(type) {
      case 'U':
        if(lengthFieldIndex >= 0) return readUnsigned((int)getFieldSize(fields[lengthFieldIndex]));
        else return readUnsigned(length);
      case 'I':
        if(lengthFieldIndex >= 0) return readSigned((int)getFieldSize(fields[lengthFieldIndex]));
        else return readSigned(length);
      case 'B':
      case 'N':
        if(length < 0) return getBytes(readUnsignedInt(1));
        else {
          if(length == 1) return getBytes(1)[0];
          else return getBytes(length);
        }
      case 'D':
        return readBits(length);
      case 'C':
      case 'S':
        if(length >= 0) return readString(length);
        else if(lengthFieldIndex >= 0) return readString((int)getFieldSize(fields[lengthFieldIndex]));
        else if(type == 'S') return readString(readUnsignedInt(2));
        else // type == 'C'
        return readString(readUnsignedInt(1));
      case 'R':
        if(length == 4) return new Float(byteArray.toFloat(getBytes(4)));
        else return new Double(byteArray.toDouble(getBytes(8)));
      case 'V':
        return readVariableTypeList();
      case 'k':
        return readArray(arrayType, arraySizeFieldIndex, lengthFieldIndex, length, fields, false);
    }
  ...
  protected Object[] readVariableTypeList() throws ParseException {
    ...
    for(int i=0; i < length; i++) {
      ...
      switch(type) {
        case 0: break; // just padding, no need to read anything
        case 1: list[i]=readUnsignedInt(1); break;
        case 2: list[i]=readUnsignedInt(2); break;
        case 3: list[i]=readUnsigned(4); break;
        case 4: list[i]=readSigned(1); break;
        case 5: list[i]=readSigned(2); break;
        case 6: list[i]=readSigned(4); break;
        case 7: list[i]=readField('R', 4, -1, ' ', -1, null); break;
        case 8: list[i]=readField('R', 8, -1, ' ', -1, null); break;
        case 10: list[i]=readField('C', -1, -1, ' ', -1, null); break;
        case 11: list[i]=readField('B', -1, -1, ' ', -1, null); break;
        case 12: list[i]=readField('D', -1, -1, ' ', -1, null); break;
        case 13: list[i]=readField('U', 1, -1, ' ', -1, null); break;
        default:
  ...
  protected Object[] readArray(char type, int fieldIndex, int lengthFieldIndex, int length, Object[] fields, boolean fillNull) throws ParseException {
    ...
    if(!fillNull) {
      if(type == 'N' && length == 1) {
        ...
        //TODO fixed this array was not filled
        // nibbles also swapped first nibble in low 4 bits
          if(++index < (count)) {
        ...
        if(lengthFieldIndex >= 0) length=(int)getFieldSize(fields[lengthFieldIndex]);
  ...
  protected byte[] readBits(int length) {
    int numBits=0;
    if(length < 0) numBits=readUnsignedInt(2);
    else numBits=length;
